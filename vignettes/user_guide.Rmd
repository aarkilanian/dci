...
Apart from calculating the DCI for potamodromous and diadromous species, `calculate_dci` can also be used to calculate an invasive form of the DCI. This invasive form uses known or predicted invaded sites within the river network (specified during the call to `river_net`) to calculate a measure of invasive connectivity. This calculation results in a global invasive DCI score from 0 to 100 indicating the risk of invasive expansion in the river network. Additionally, each segment receives a segment-level DCI indicating the risk of that specific segment being invaded by existing invasive populations.
