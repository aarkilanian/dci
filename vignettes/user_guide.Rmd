...
##### Talk about the addition of the force argument. And be honest about problems that might arise from multiple points on a river or if points are too close to the end vertices of lines. force is an argument which would allow for "legal" nodes to be incorporated into the river network while simply ignoring those which are problematic, effectively removing them from the network and subsequent dci calculations. At the moment, there is no method to identify which points are problematic.
...
```{r}
# Loading river sf object
load("../data/yamaska_riv.RData")

# Importing rivers from a shapefile
rivers_in <- import_rivers(rivers = yamaska_riv)
```
...
```{r}
# Loading sf objects
load("../data/yamaska_bar.RData")
load("../data/yamaska_outlet.RData")
load("../data/yamaska_poi.RData")

# Import barriers
barriers_in <- import_points(pts = yamaska_bar, type = "barriers")
outlet_in <- import_points(pts = yamaska_outlet, type = "outlet")
poi_in <- import_points(pts = yamaska_poi, type = "poi")
barriers_in
```
